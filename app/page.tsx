...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-16" data-reveal>
      <h2 className="mb-4 text-xs font-bold uppercase tracking-[0.15em]" style={{ color: "var(--cr-text-dim)" }}>{title}</h2>
      {children}
    </section>
  );
}
    ...
    <div style={{ position: "relative", minHeight: "100vh" }}>
      ...
      <main className="mx-auto max-w-4xl px-6 py-20" style={{ position: "relative" }}>
        <header className="mb-16" data-reveal>
          <p className="cr-display mb-2 text-xs font-bold uppercase tracking-[0.15em]" style={{ color: "var(--cr-accent)" }}>Catálogo Crystal</p>
          <h1 className="cr-display text-4xl font-bold" style={{ color: "var(--cr-text)" }}>Liquid glass premium</h1>
          <p className="mt-2 max-w-lg" style={{ color: "var(--cr-text-dim)" }}>
            Vidrio con distorsión SVG real, mesh vivo que respira, háptica y luz que sigue el cursor. Todo el kit que armaste, ya como componentes React.
          </p>
        </header>
        ...
        <Section title="Vidrio líquido">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <GlassCard>
              <p className="cr-display mb-1 text-lg font-semibold" style={{ color: "var(--cr-text)" }}>Blur normal</p>
              <p style={{ color: "var(--cr-text-dim)" }}>backdrop-filter: blur + saturate.</p>
            </GlassCard>
            <GlassCard liquid>
              <p className="cr-display mb-1 text-lg font-semibold" style={{ color: "var(--cr-text)" }}>Distorsión real</p>
              <p style={{ color: "var(--cr-text-dim)" }}>url(#lg-dist) — dobla la luz de verdad, corre en Chromium.</p>
            </GlassCard>
          </div>
        </Section>
        ...
        <Section title="Toggle, Segmented, Chip">
          <div className="flex flex-wrap items-center gap-8">
            ...
            <Segmented value={period} onChange={setPeriod} options={[{ value: "day", label: "Día" }, { value: "week", label: "Semana" }]} />
            <Chip active={chip} onClick={() => setChip((c) => !c)}>Glass</Chip>
          </div>
        </Section>
        ...
        <Section title="Pricing (borde vivo)">
          <PricingCard className="max-w-xs">
            <p className="cr-display text-sm font-bold" style={{ color: "var(--cr-accent)" }}>PRO</p>
            ...
            <p className="mb-4 text-sm" style={{ color: "var(--cr-text-dim)" }}>Todo el kit, uso ilimitado.</p>
            <CrystalButton variant="cta" style={{ width: "100%" }}>Elegir plan</CrystalButton>
          </PricingCard>
        </Section>
        ...
        <Section title="Gooey (estilo Telegram)">
          <p className="mb-3 text-sm" style={{ color: "var(--cr-text-dim)" }}>Mueve el mouse — los blobs se funden al acercarse.</p>
          <GooeyBlobs className="max-w-sm" />
        </Section>

        <Section title="Modal">
          <CrystalButton variant="ghost" onClick={() => setModalOpen(true)}>Abrir modal</CrystalButton>
          <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
            <p className="cr-display mb-2 text-lg font-semibold" style={{ color: "var(--cr-text)" }}>¿Confirmar?</p>
            <p className="mb-5 text-sm" style={{ color: "var(--cr-text-dim)" }}>Cierra con tap fuera o Escape.</p>
            <div className="flex justify-end gap-2">
              <CrystalButton variant="ghost" onClick={() => setModalOpen(false)}>Cancelar</CrystalButton>
              <CrystalButton variant="cta" onClick={() => setModalOpen(false)}>Confirmar</CrystalButton>
            </div>
          </Modal>
        </Section>
      </main>
