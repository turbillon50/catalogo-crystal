...
// Uso: import { initCrystal } from './crystal-ui'; useEffect(() => initCrystal(), []);
export function initCrystal(opts = {}) {
  const cleanups = [];
  // 1. Háptica: thump spring en cada press + vibración real en móvil
  const down = (e) => {
  ...
  // 2. Vidrio vivo: la turbulencia del filtro #lg-dist respira
  ...
  // 3. Luz especular que sigue al cursor + parallax del mesh
  ...
  // 4. Coreografía de entrada: stagger 75ms con blur→nítido en [data-reveal]
  ...
  return () => cleanups.forEach(fn => fn()); // cleanup para useEffect
}

// Squash & stretch (jelly, estilo WhatsApp) — llama en onClick/onPointerDown
